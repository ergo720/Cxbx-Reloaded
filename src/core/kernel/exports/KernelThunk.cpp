...
// kernel thunk table
// NOTE1 : Names that collide with other symbols, use the KRNL() macro.
// NOTE2 : These wrappers are needed because the xbox pointer type is implemented with a class, and msvc passes those via a hidden pointer argument, while the guest
//         will expect to receive the value of the pointer as is, instead of a pointer to an object.
// NOTE3 : These wrappers are only needed with direct exec. With cou emulation, we will be able to specify the correct type directly
// NOTE4 : All variadic functions cannot use wrappers becasue it's not possible to forward the variadic arguments as is, unless we are willing to change the signature
//         to use a va_list instead of ...
// NOTE5 : The rtl exception functions cannot use wrappers because it will add an extra call frame that should not be there, which will be counted as well, and cause
//         the functions to return wrong results
...
uint32_t CxbxKrnl_KernelThunkTable[379] =
{
	...
	(uint32_t)FUNC(&xbox::WRAP(ExAllocatePool)),                    // 0x000E (14)
	(uint32_t)FUNC(&xbox::WRAP(ExAllocatePoolWithTag)),             // 0x000F (15)
	...
	(uint32_t)FUNC(&xbox::WRAP(RtlAssert)),                         // 0x0108 (264)
	...
	(uint32_t)FUNC(&xbox::WRAP(RtlMultiByteToUnicodeN)),            // 0x012B (299)
	(uint32_t)FUNC(&xbox::WRAP(RtlMultiByteToUnicodeSize)),         // 0x012C (300)
	...
	(uint32_t)FUNC(&xbox::WRAP(RtlUnicodeToMultiByteN)),            // 0x0136 (310)
	...
	(uint32_t)FUNC(&xbox::WRAP(RtlUpcaseUnicodeToMultiByteN)),      // 0x013B (315)
	...
	(uint32_t)FUNC(&xbox::WRAP(RtlVsnprintf)),                      // 0x016B (363)
	(uint32_t)FUNC(&xbox::WRAP(RtlVsprintf)),                       // 0x016C (364)
